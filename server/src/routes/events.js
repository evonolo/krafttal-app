...
eventsRouter.get('/', requireLogin, (req, res) => {
  ...
  res.json({ events: rows.map((r) => baueEvent(r, req.user.id)) });
});
...
eventsRouter.get('/:id', requireLogin, (req, res) => {
  ...
  res.json({ event: baueEvent(row, req.user.id) });
});
...
eventsRouter.post('/', requireActive, (req, res) => {
  ...
  res.status(201).json({ event: baueEvent(row, req.user.id) });
});
